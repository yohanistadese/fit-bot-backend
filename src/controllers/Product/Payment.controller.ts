...
import { PaymentService } from "../../services/Product";
...
class PaymentController {
  ...
  static create(request: any, response: Response) {
    ...
    const schema = Joi.object({
      ...
      stripe_charge_id: Joi.string().allow(null, ""),
      amount: Joi.number().required(),
      currency: Joi.string().required(),
      status: Joi.string()
        .valid("PENDING", "COMPLETED", "FAILED")
        .default("PENDING"),
      raw_event: Joi.object().allow(null),
    });

    const { error } = schema.validate(request.body, { abortEarly: false });

    if (!error) {
      PaymentService.create(request.user, request.body)
        .then((result) =>
  ...
  static update(request: any, response: Response) {
    ...
    if (!error) {
      ...
      PaymentService.update(request.user, id, data)
        .then((result) =>
  ...
  static delete(request: any, response: Response) {
    ...
    if (!error) {
      ...
      PaymentService.delete(request.user, id, null, force ?? false)
        .then((result) =>
          ServerResponse(request, response, 200, result, "Success", startTime)
        )
        .catch((err) =>
  ...
  static restore(request: any, response: Response) {
    ...
    if (!error) {
      ...
      PaymentService.restore(request.user, id)
        .then((result) =>
          ServerResponse(request, response, 200, result, "Success", startTime)
        )
        .catch((err) =>
