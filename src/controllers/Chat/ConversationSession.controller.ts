...
import ConversationSessionService from "../../services/Chat/ConversationSession.service";

class ConversationSessionController {
  static ModelName = "Conversation Session";

  static findMany(request: any, response: Response) {
    const startTime = new Date();
    const parsedQuery: any = ParseQuery(request.query);
    ConversationSessionService.findMany(
      request.user,
      parsedQuery.query,
  ...
  static findOne(request: any, response: Response) {
    const startTime = new Date();
    const parsedQuery: any = ParseQuery(request.query);
    ConversationSessionService.findOne(
      request.user,
      parsedQuery.query,
      parsedQuery.paranoid
    )
  ...
  static findById(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const parsedQuery: any = ParseQuery(request.query);
      ConversationSessionService.findById(
        request.user,
        id,
        parsedQuery.query,
        parsedQuery.paranoid
  ...
  static create(request: any, response: Response) {
    ...
    if (!error) {
      ConversationSessionService.create(request.user, request.body)
        .then((result) =>
  ...
  static update(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ConversationSessionService.update(request.user, id, data)
        .then((result) =>
  ...
  static delete(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ConversationSessionService.delete(request.user, id, null, force ?? false)
        .then((result) =>
          ServerResponse(request, response, 200, result, "Success", startTime)
        )
        .catch((err) =>
  ...
  static restore(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ConversationSessionService.restore(request.user, id)
        .then((result) =>
          ServerResponse(request, response, 200, result, "Success", startTime)
        )
        .catch((err) =>
