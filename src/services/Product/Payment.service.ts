...
import { Payment } from "../../models/Product";
import { PaymentDAL } from "../../dals/Product";
...
class PaymentService {
  static create = (
    user: User,
    payload: Omit<Payment, NullishPropertiesOf<Payment>>
  ): Promise<Payment> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            PaymentDAL.create(payload)
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      PaymentDAL.findOne(options, paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      PaymentDAL.findMany(options, paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      PaymentDAL.findById(id, options, paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            PaymentService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: Payment, done: Function) => {
            const prev = { ...obj.toJSON() };
            PaymentDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: Payment, done: Function) => {
            PaymentDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: Payment, done: Function) => {
            PaymentDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
