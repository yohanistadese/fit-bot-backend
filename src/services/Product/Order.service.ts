...
import { OrderDAL } from "../../dals/Product";
...
class OrderService {
  static create = (
    user: User,
    payload: Omit<Order, NullishPropertiesOf<Order>>
  ): Promise<Order> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            OrderDAL.create(payload)
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      OrderDAL.findOne(options, paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      OrderDAL.findMany(options, paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      OrderDAL.findById(id, options, paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            OrderService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: Order, done: Function) => {
            const prev = { ...obj.toJSON() };
            OrderDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: Order, done: Function) => {
            OrderDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: Order, done: Function) => {
            OrderDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
