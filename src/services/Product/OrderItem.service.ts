...
import { OrderItem } from "../../models/Product";
import { OrderItemDAL } from "../../dals/Product";
...
class OrderItemService {
  static create = (
    user: User,
    payload: Omit<OrderItem, NullishPropertiesOf<OrderItem>>
  ): Promise<OrderItem> => {
            ...
            OrderItemDAL.findOne({
              ...
              .then((existing) => {
                if (existing)
                    ...
                    new BadRequestError([
                      `${ModelName} already exists in order`,
                    ])
                  );
                else done(null);
              })
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      OrderItemDAL.findOne(options, paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      OrderItemDAL.findMany(options, paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      OrderItemDAL.findById(id, options, paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            OrderItemService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: OrderItem, done: Function) => {
            const prev = { ...obj.toJSON() };
            OrderItemDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: OrderItem, done: Function) => {
            OrderItemDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: OrderItem, done: Function) => {
            OrderItemDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
