...
import { ProductDAL } from "../../dals/Product";
  ...
  static create = (
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            ProductDAL.findOne({ where: { slug: payload.slug } })
              .then((existing) => {
                if (existing)
                  done(
                    new BadRequestError([`${ModelName} slug already exists`])
                  );
                else done(null);
              })
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ProductService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: Product, done: Function) => {
            const prev = { ...obj.toJSON() };
            ProductDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: Product, done: Function) => {
            ProductDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, obj: Product, done: Function) => {
            ProductDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
