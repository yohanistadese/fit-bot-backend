...
import { ConversationSession } from "../../models/Chat";
...
import { ConversationSessionDAL } from "../../dals/Chat";

const ModelName = "ConversationSession";
...
class ConversationSessionService {
  ...
  static create = (
    user: any,
    payload: Omit<ConversationSession, NullishPropertiesOf<ConversationSession>>
  ): Promise<ConversationSession> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            ConversationSessionDAL.findOne({ where: { id: payload.id } })
              .then((existing) => {
                if (existing) {
                  done(
                    new BadRequestError([
          ...
          (done: Function) => {
            ConversationSessionDAL.create(payload)
  ...
  static findOne = (user: any, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = ConversationSessionService.AuthOptions(
        user,
        options,
        paranoid
      );
      ConversationSessionDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
  ...
  static findMany = (user: any, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      ...
      ConversationSessionDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      ...
      ConversationSessionDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ConversationSessionService.findById(user, id, options)
              .then((obj) => {
          ...
          (
            ...
            ConversationSessionDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ConversationSessionService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (
            ...
            ConversationSessionDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ConversationSessionService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (
            ...
            ConversationSessionDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
