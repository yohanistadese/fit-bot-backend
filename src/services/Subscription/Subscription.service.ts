...
import { SubscriptionDAL } from "../../dals/Subscription";
...
class SubscriptionService {
  ...
  static create = (
    user: User,
    payload: Omit<Subscription, NullishPropertiesOf<Subscription>>
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = SubscriptionService.AuthOptions(user, options, paranoid);
      SubscriptionDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = SubscriptionService.AuthOptions(user, options, paranoid);
      SubscriptionDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      const auth = SubscriptionService.AuthOptions(user, options, paranoid);
      SubscriptionDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            SubscriptionService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: Subscription, done: Function) => {
            const prev = { ...obj.toJSON() };
            SubscriptionDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            SubscriptionService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (transaction: Transaction, obj: Subscription, done: Function) => {
            SubscriptionDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            SubscriptionService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (transaction: Transaction, obj: Subscription, done: Function) => {
            SubscriptionDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
