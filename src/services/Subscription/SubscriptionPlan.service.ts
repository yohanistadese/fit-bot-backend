...
import { SubscriptionPlanDAL } from "../../dals//Subscription";
...
class SubscriptionPlanService {
  ...
  static create = (
    user: User,
    payload: Omit<SubscriptionPlan, NullishPropertiesOf<SubscriptionPlan>>
  ): Promise<SubscriptionPlan> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            SubscriptionPlanDAL.findOne({ where: { name: payload.name } })
              .then((existing) => {
                if (existing) {
                  done(
                    new BadRequestError([
                      `${ModelName} '${payload.name}' already exists`,
                    ])
                  );
                } else done(null);
              })
          ...
          (done: Function) => {
            SubscriptionPlanDAL.create(payload)
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            if (payload.name) {
              SubscriptionPlanDAL.findOne({
                where: { name: payload.name, id: { $ne: id } },
              })
                .then((existing) => {
                  if (existing) {
                    done(
                  ...
                  } else done(null, transaction);
                })
                .catch((error) => done(new InternalServerError(error)));
            } else done(null, transaction);
          },
          (transaction: Transaction, done: Function) => {
            SubscriptionPlanService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: SubscriptionPlan, done: Function) => {
            const prev = { ...obj.toJSON() };
            SubscriptionPlanDAL.update(obj, payload, transaction)
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = SubscriptionPlanService.AuthOptions(user, options, paranoid);
      SubscriptionPlanDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = SubscriptionPlanService.AuthOptions(user, options, paranoid);
      SubscriptionPlanDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      const auth = SubscriptionPlanService.AuthOptions(user, options, paranoid);
      SubscriptionPlanDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            SubscriptionPlanService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (transaction: Transaction, obj: SubscriptionPlan, done: Function) => {
            SubscriptionPlanDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            SubscriptionPlanService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (transaction: Transaction, obj: SubscriptionPlan, done: Function) => {
            SubscriptionPlanDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
