...
class UserProfileService {
  ...
  static create = (
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            UserProfileDAL.findOne({ where: { user_id: payload.user_id } })
              .then((existing) => {
                if (existing) {
                  done(
                    new BadRequestError([
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = UserProfileService.AuthOptions(user, options, paranoid);
      UserProfileDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = UserProfileService.AuthOptions(user, options, paranoid);
      UserProfileDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      const auth = UserProfileService.AuthOptions(user, options, paranoid);
      UserProfileDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            UserProfileService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: UserProfile, done: Function) => {
            const prev = { ...obj.toJSON() };
            UserProfileDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            UserProfileService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (transaction: Transaction, obj: UserProfile, done: Function) => {
            UserProfileDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            UserProfileService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else
                  done(new NotFoundError(`${ModelName} Not Found`), {
          ...
          (transaction: Transaction, obj: UserProfile, done: Function) => {
            UserProfileDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) =>
                done(new InternalServerError(e), { obj: null, transaction })
              );
