...
import { PlanDay } from "../../models/Fitness";
...
class PlanDayService {
  ...
  static create = (
    user: User,
    payload: Omit<PlanDay, NullishPropertiesOf<PlanDay>>
  ): Promise<PlanDay> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            WeeklyPlanDAL.findOne({ where: { id: payload.weekly_plan_id } })
              .then((weeklyPlan) => {
                if (!weeklyPlan) {
                  done(new NotFoundError("Weekly Plan not found"));
                } else done(null);
              })
            ...
            PlanDayDAL.findOne({
              ...
              .then((existing) => {
                if (existing) {
                    ...
                    new BadRequestError([
                      `${ModelName} for this date already exists`,
                    ])
                  );
                } else done(null);
              })
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      ...
      PlanDayDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = PlanDayService.AuthOptions(user, options, paranoid);
      PlanDayDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      const auth = PlanDayService.AuthOptions(user, options, paranoid);
      PlanDayDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            PlanDayService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: PlanDay, done: Function) => {
            const prev = { ...obj.toJSON() };
            PlanDayDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            PlanDayService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else done(new NotFoundError(`${ModelName} Not Found`));
              })
          ...
          (transaction: Transaction, obj: PlanDay, done: Function) => {
            PlanDayDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            PlanDayService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else done(new NotFoundError(`${ModelName} Not Found`));
              })
          ...
          (transaction: Transaction, obj: PlanDay, done: Function) => {
            PlanDayDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) => done(new InternalServerError(e)));
          },
          (obj: any, result: any, done: Function) => {
