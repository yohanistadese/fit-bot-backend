...
class ExerciseService {
  ...
  static create = (
    user: User,
    payload: Omit<Exercise, NullishPropertiesOf<Exercise>>
  ): Promise<Exercise> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            ExerciseDAL.findOne({ where: { slug: payload.slug } })
              .then((existing) => {
                if (existing) {
                  done(
                    new BadRequestError([
                      `${ModelName} with this slug already exists`,
                    ])
                  );
                } else done(null);
              })
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      ...
      ExerciseDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = ExerciseService.AuthOptions(user, options, paranoid);
      ExerciseDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      const auth = ExerciseService.AuthOptions(user, options, paranoid);
      ExerciseDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ExerciseService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: Exercise, done: Function) => {
            const prev = { ...obj.toJSON() };
            ExerciseDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ExerciseService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else done(new NotFoundError(`${ModelName} Not Found`));
              })
          ...
          (transaction: Transaction, obj: Exercise, done: Function) => {
            ExerciseDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            ExerciseService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else done(new NotFoundError(`${ModelName} Not Found`));
              })
          ...
          (transaction: Transaction, obj: Exercise, done: Function) => {
            ExerciseDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) => done(new InternalServerError(e)));
          },
          (obj: any, result: any, done: Function) => {
