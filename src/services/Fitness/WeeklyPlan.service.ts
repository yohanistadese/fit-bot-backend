...
import { WeeklyPlan } from "../../models/Fitness";
...
class WeeklyPlanService {
  ...
  static create = (
    user: User,
    payload: Omit<WeeklyPlan, NullishPropertiesOf<WeeklyPlan>>
  ): Promise<WeeklyPlan> => {
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          (done: Function) => {
            WeeklyPlanDAL.findOne({
              where: {
              ...
              .then((existing) => {
                if (existing)
                    ...
                    new BadRequestError([
                      `${ModelName} already exists for this start date`,
                    ])
                  );
                else done(null);
              })
  ...
  static findOne = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = WeeklyPlanService.AuthOptions(user, options, paranoid);
      WeeklyPlanDAL.findOne(auth.options, auth.paranoid)
        .then(resolve)
  ...
  static findMany = (user: User, options: any, paranoid?: boolean) => {
    return new Promise((resolve, reject) => {
      const auth = WeeklyPlanService.AuthOptions(user, options, paranoid);
      WeeklyPlanDAL.findMany(auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
  ...
  static findById = (
    ...
    return new Promise((resolve, reject) => {
      const auth = WeeklyPlanService.AuthOptions(user, options, paranoid);
      WeeklyPlanDAL.findById(id, auth.options, auth.paranoid)
        .then(resolve)
        .catch((e) => reject(new InternalServerError(e)));
    });
  };
      ...
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            WeeklyPlanService.findById(user, id, options)
              .then((obj) => {
          ...
          (transaction: Transaction, obj: WeeklyPlan, done: Function) => {
            const prev = { ...obj.toJSON() };
            WeeklyPlanDAL.update(obj, payload, transaction)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            WeeklyPlanService.findById(user, id, options, force)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else done(new NotFoundError(`${ModelName} Not Found`));
              })
          ...
          (transaction: Transaction, obj: WeeklyPlan, done: Function) => {
            WeeklyPlanDAL.delete({ id: obj.id }, transaction, force)
    ...
    return new Promise((resolve, reject) => {
      async.waterfall(
        [
          ...
          (transaction: Transaction, done: Function) => {
            WeeklyPlanService.findById(user, id, options, true)
              .then((obj) => {
                if (obj) done(null, transaction, obj);
                else done(new NotFoundError(`${ModelName} Not Found`));
              })
          ...
          (transaction: Transaction, obj: WeeklyPlan, done: Function) => {
            WeeklyPlanDAL.restore({ id: obj.id }, transaction)
              .then((result) => done(null, obj, { obj: result, transaction }))
              .catch((e) => done(new InternalServerError(e)));
          },
          (obj: any, result: any, done: Function) => {
